	...
	case 4:
		{
			...
			if (responseObject.get("result", false).asBool()) {
				CStyle style(m_hConsole, 10);
				cnote << EthLime "**Submitted and Accepted" EthReset " in" << ms.count() << "ms.";
				p_farm->acceptedSolution(m_stale);
			}
			else {
				CStyle style(m_hConsole, 12);
