...
*/
		...
		switch (m_conn.Version()) {
			...
			case EthStratumClient::ETHEREUMSTRATUM:
				...
				jReq["params"].append("ethminer for " ETH_BUILD_PLATFORM " " ETH_BUILD_TYPE);
...
void EthStratumClient::processReponse(Json::Value& responseObject)
{
	...
	if (!_isNotification) {
		...
		case 3:
			...
			if (!m_authorized)
			{
				cwarn << "Worker not authorized" << m_conn.User() << _errReason;
				disconnect();
				return;		
			}
			else {
				if (m_conn.FeeName().length() == 0)
				{
					cnote << "Authorized worker " << EthWhite << m_conn.User() << EthReset;
				}
				else
				{
					size_t p1 = m_conn.User().find(m_conn.FeeName());
					if (p1 != string::npos)
						cnote << EthCyan << "APPRECIATION: You are now helping us improve these products for you - Thank You!" << EthReset;
					else
						cnote << EthWhite << "Authorized worker " << m_conn.User() << EthReset;
				}
			}
	...
	if (_isNotification) {
		...
		else if (_method == "client.get_version")
		{
			...
			jReq["result"] = ETH_BUILD_PLATFORM " " ETH_BUILD_TYPE;
