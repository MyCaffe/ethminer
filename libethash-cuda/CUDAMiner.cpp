...
extern int* g_pnGpuThrottle;
extern int* g_pnGpuLastThrottle;
...
bool CUDAMiner::init(const h256& seed)
{
	try {
		...
		cnote << "Initializing miner " << index;
...
void CUDAMiner::status(double dfPct)
{
	//cudalog << "DAG Initialization at " << fixed << std::setprecision(2) << dfPct << "%";
}
		...
		if(dagSize128 != m_dag_size || !dag)
		{
			...
			if (!hostDAG)
			{
				if((m_device_num == dagCreateDevice) || !_cpyToHost){ //if !cpyToHost -> All devices shall generate their DAG
					...
					ethash_generate_dag(dagSize, s_gridSize, s_blockSize, m_streams[0], m_device_num, &status);
	...
	const dev::eth::WorkPackage& w)
{
	...
	while (true)
	{
		...
		{
			int nCount = g_pnGpuThrottle[0];

			if (m_device_num < nCount && nCount <= MAX_GPU)
			{
				int nThrottle = g_pnGpuThrottle[m_device_num + 1];
				if (nThrottle > 0)
				{
					if (nThrottle > 1000)
						nThrottle = 1000;

					this_thread::sleep_for(chrono::milliseconds(nThrottle));

					if (g_pnGpuLastThrottle != NULL)
					{
						if (g_pnGpuLastThrottle[m_device_num] != nThrottle)
						{
							float fThrtl = (1.0f - ((float)nThrottle / 1000.0f)) * 100.0f;
							char szBuffer[256];
							snprintf(szBuffer, 256, "GPU %d: throttling set to %0.2f %%", m_device_num, fThrtl);
							cwarn << szBuffer;
							g_pnGpuLastThrottle[m_device_num] = nThrottle;
						}
					}
				}
			}
