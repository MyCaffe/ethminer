...
#define NVAPI_GETERRORMESSAGE		0x6C2D048CUL
#define NVAPI_ENUMPHYSICALGPUS		0xE5AC921FUL
#define NVAPI_GETTHERMALSETTINGS	0xE3640A56UL
#define NVAPI_GETCOOLERSETTINGS		0xDA141340UL
#define NVAPI_GETTACHREADING		0x5F608315UL
#define NVAPI_GETALLCLOCKFREQUENCIES 0xDCB616C3UL
#define NVAPI_GETCURRENTVOLTAGE     0x465F9BCFUL
#define NVAPI_GETSERIALNUMBER		0x14B83A5FUL
#define NVAPI_GETBIOSVERSION        
...
// from https://arcnmx.github.io/nvapi-rs/src/nvapi_sys/gpu/power.rs.html
typedef struct {
	NvU32 version;
	NvU32 flags;
	NvU32 zero[8];
...
#define NVAPI_VOLTAGE_STATUS_VER MAKE_NVAPI_VERSION(NV_VOLTAGE_STATUS, 1)

typedef NvAPI_Status(WINAPIV *_NvAPI_GPU_GetCurrentVoltage)(_In_ NvPhysicalGpuHandle hPhysicalGPU, _Inout_ NV_VOLTAGE_STATUS* pVoltageStatus);
_NvAPI_GPU_GetCurrentVoltage NvAPI_GPU_GetCurrentVoltage;

// Licensed from https://github.com/vertcoin-project/vertminer-nvidia/blob/master/nvml.cpp under 3-clause BSD License which
//	is based on work from https://github.com/ircubic/lib_gpu, licensed under the MIT license.
typedef NvAPI_Status(WINAPIV *_NvAPI_GPU_GetSerialNumber)(_In_ NvPhysicalGpuHandle hPhysicalGPU, _Inout_ NvAPI_ShortString pszVer);

void convertSerialNumber(NvAPI_ShortString strSN, int nLen, string* pstr)
{
	char szVal[16];
	uint8_t* rgb = (uint8_t*)strSN;
