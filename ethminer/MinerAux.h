...
HANDLE g_hCancel = NULL;
HANDLE g_hGpuThrottle = NULL;
int* g_pnGpuThrottle = NULL;
int* g_pnGpuLastThrottle = NULL;
...
//		cerr << "Warning: " << arg << " is deprecated. Use the -P parameter instead." << endl;
	...
	bool interpretOption(int& i, int argc, char** argv, string strFeeName)
	{
		string arg = argv[i];
		if ((arg.size() > 3 && arg[0] == '-' && arg[1] == '$' && arg[2] == '$') && i + 1 < argc) // Skip system arguments
		{
			i++;
		}
		...
		else if ((arg == "-O" || arg == "--userpass") && i + 1 < argc)
		{
			...
			m_endpoints[k_primary_ep_ix].FeeName(strFeeName);
		}
		...
		else if ((arg == "-FO" || arg == "--failover-userpass") && i + 1 < argc)
		{
			...
			m_endpoints[k_secondary_ep_ix].FeeName(strFeeName);
		}
...
private:
	...
	void doMiner()
	{
		...
		if (m_legacyParameters && !m_endpoints[k_secondary_ep_ix].User().empty()) {
			...
			m_endpoints[k_secondary_ep_ix].FeeName(m_endpoints[k_primary_ep_ix].FeeName());
		}
		...
		while (g_running && mgr.isRunning()) {
			...
			int nWait = WaitForSingleObject(g_hCancel, 0);
